...
$$
p(\beta | y_1, \ldots, y_n) \propto \prod_{j=0}^p\exp\left(\frac{1}{2\sigma_j^2}(\beta_j - \mu_j)^2\right) \prod_{i=1}^{n} q\left(x_i\right)^{y_i}\left(1-q\left(x_i\right)\right)^{1-y_i}.
$$
Taking the logarithm gives

$$
\log p(\beta | y_1, \ldots, y_n) \propto \sum_{j=0}^p \frac{1}{2\sigma_j^2}(\beta_j - \mu_j)^2 +  \sum_{i=1}^{n} {y_i}\log q\left(x_i\right) + (1 - y_i) \log \left(1-q\left(x_i\right)\right)
$$
...
# 2.3. (Divide-and-Conquer with) Sequential Monte Carlo
...
For the submodels the linear predictors are, respectively 
...
The link parameter is $\phi = \beta_5$ and model specific parameters are $\psi_1 = (\beta_{0,1}, \beta_1, \beta_6)$ and $\psi_2 = (\beta_{0,2}, \beta_3, \beta_4)$. Both submodels have the same observable random variables $Y_1 = Y_2 = Y$, the response variable `pos`.
...
$$
Taking a normal prior as in Section 1 for $\beta_{1:5}$, and similarly for the intercepts
...
&\propto \exp\left(\frac{1}{2\sigma_{0,1}^2}(\beta_{0,1} - \mu_{0,1})^2\right) \prod_{j=1,4,5}\exp\left(\frac{1}{2\sigma_j^2}(\beta_j - \mu_j)^2\right) \prod_{i=1}^{2035} q_1\left(x_i\right)^{y_i}\left(1-q_1\left(x_i\right)\right)^{1-y_i}
\end{align*}
...
&\propto \exp\left(\frac{1}{2\sigma_{0,2}^2}(\beta_{0,2} - \mu_{0,2})^2\right) \prod_{j=2,3,5}\exp\left(\frac{1}{2\sigma_j^2}(\beta_j - \mu_j)^2\right) \prod_{i=1}^{2035} q_2\left(x_i\right)^{y_i}\left(1-q_2\left(x_i\right)\right)^{1-y_i}
\end{align*}
...
&= \frac{p_1(\beta_{0, 1}, \beta_1, \beta_4, \beta_5 | \mathbf{y}) p_2(\beta_{0, 2}, \beta_2, \beta_3, \beta_5 | \mathbf{y})}{p(\beta_5)} \\
...
&\times \prod_{i=1}^{2035} q_1\left(x_i\right)^{y_i}\left(1-q_1\left(x_i\right)\right)^{1-y_i} q_2\left(x_i\right)^{y_i}\left(1-q_2\left(x_i\right)\right)^{1-y_i} 
\end{align*}

*Informal: almost like Bayesian inference for full model with likelihood squared but not quite*
