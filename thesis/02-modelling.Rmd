...
This is particularly pertinent if the observable data $Y_m$ are from different background areas such that the modelling process naturally decomposes via consultation of different domain experts for each data source who specify the submodels.
In this situation, the conditional independence assumption also seems, in general, quite reasonable.
In general, it is worth noting that conditional independence statements are hard to test [@shah2018hardness] and therefore often their validitity will rest on more informal "common sense" arguements.
...
The structure of Equation \eqref{eq:comb} is then explained by applying these two assumptions - conditional indepedence and prior consistency - to a hypothetical joint model
\begin{align}
...
We present the following pedagogical example of Markov melding with $M = 2$ submodels which, as with Example 2.1, uses conjugacy to enable exact Bayesian computation.
...
**Submodel 1** 
Define the first submodel $p_1(\theta, Y_1)$ as
\begin{align}
...
Figure \ref{fig:ex1} part (b) shows that this submodel, both the prior and posterior, are misguided with respect to $\theta$.
...
To extend into more realistic situations, in the following chapter we will introduce statistical simulation techniques that allow inference to be conducted on generic Markov melded posterior distributions.
