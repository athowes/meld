...
$$
R\left(\phi, \psi_{1}, \ldots, \psi_{M}, \phi^\star, \psi_{1}, \ldots, \psi_{M}\right) = p_{\mathrm{pool}}\left(\phi^{\star}\right) \prod_{m=1}^{M} \frac{p_{m}\left(\phi^{\star}, \psi_{m}, y_{m}\right)}{p_{m}\left(\phi^{\star}\right)} \times \frac{1}{q\left(\phi^{\star} | \phi\right)}
$$
...
$$
p_{\text {meld,}l}(\phi, \psi_{1}, \ldots, \psi_{\ell} | y_{1}, \ldots, y_{\ell}\right) \propto \prod_{m=1}^{\ell}(\frac{p_{m}(\phi, \psi_{m}, y_{m}\right)}{p_{m}(\phi)} p_{\mathrm{pool}, m}(\phi)\right)
$$
...
**Stage $l$.** construct a Metropolis-within-Gibbs sampler for $\left(\phi, \psi_1, \ldots, \psi_\ell\right)$ given the data $\left(y_1, \ldots, y_\ell\right)$. The parameter $\psi_\ell$ is updated, conditional on the link parameter $\phi$ and parameters $\psi_1, \ldots, \psi_{\ell-1}$ using a Metropolis-Hastings step (say) with target-to-proposal density ratio

$$
R\left(\phi, \psi_{1}, \ldots, \psi_{\ell}^{\star}, \phi, \psi_{1}, \ldots, \psi_{\ell}\right) = p_{\ell}\left(\phi, \psi_{\ell}^{\star}, y_{\ell}\right) \times \frac{1}{q\left(\psi_{\ell}^{\star} | \phi, \psi_{\ell}\right)}
$$


The samples from stage $\ell-1$ are used as a proposal to update $\phi_1, \ldots, \phi_{\ell-1}$ and the link parameter $\phi$. The amounts to sampling with replacement, i.e. drawing an index $d \sim \mathcal{U}(\{1, \ldots, H_{\ell - 1}\})$ and setting $\left(\phi^{\star}, \psi_{1}^{\star}, \ldots, \psi_{\ell-1}^{\star}\right)=\left(\phi^{(d, \ell-1)}, \psi_{1}^{(d, \ell-1)}, \ldots, \psi_{\ell-1}^{(d, \ell-1)}\right)$

$$
R\left(\phi, \psi_{1}^\star, \ldots, \psi_{\ell-1}^\star, \psi_{\ell}, \phi, \psi_{1}, \ldots, \psi_{\ell-1}, \psi_{\ell}\right) = \frac{p_{\ell}\left(\phi^{\star}, \psi_{\ell}, y_{\ell}\right)}{p_{\ell}\left(\phi^{\star}\right)} p_{\text{pool}, \ell}\left(\phi^{\star}\right)
$$
Once sampling in stage $\ell$ has converged, the samples $\left(\phi^{(h, \ell)}, \psi_{1}^{(h, \ell)}, \ldots, \psi_{\ell}^{(h, \ell)}\right), h=1, \ldots, H_{\ell}$ are obtained for use in stage $\ell + 1$

### Estimating marginal distributions

* $p_m(\phi)$ might be directly-specified as a founder node in the submodel's DAG
* Else, estimate by KDE with samples drawn from $p_m(\phi)=\iint p_m\left(\phi, \psi_m, Y_m\right) d \psi_m d Y_m$
