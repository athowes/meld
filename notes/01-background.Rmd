...
* Monte Carlo methods "represent the solution of a problem as a parameter of a hypothetical population, and using a random sequence of numbers to construct a sample of the population, from which statistical estimates of the parameter can be obtained"
* Mainly want to approximate a generic probability density $\pi(x)$
* If it is possible to sample $X_i \sim \pi(x)$ for $i = 1,\ldots, N$ then Monte Carlo approximates $\pi(x)$ by the empirical measure $\widehat{\pi}(x) = \frac{1}{N} \sum_{i=1}^{N} \delta_{X_i}(x)$
* Can approximate the expectation of any test function $\varphi: \mathcal{X} \to \mathbb{R}$ given by $\mathbb{E}_\pi \varphi(x) := \int \varphi(x) \pi(x) dx$ as $\int \varphi(x) \widehat{\pi}(x) dx = \frac{1}{N} \sum_{i=1}^{N} \varphi(X_i)$
...
1. Draw $j \sim \text{Unif}\{1, \ldots, p\}$
2. Draw $x_{j} \sim q_j\left(\cdot | x^{(t-1)}\right)$ and set $x = \left(x_{1}^{(t-1)}, \ldots, x_j, \ldots,  x_{p}^{(t-1)}\right)$
3. With probability $\min \left\{1, \frac{f(x) \cdot q\left(x^{(t-1)} | x\right)}{f\left(x^{(t-1)}\right) \cdot q\left(x | x^{(t-1)}\right)}\right\}$ set $x^{(t)}=x$, else set $x^{(t)}=x^{(t-1)}$
