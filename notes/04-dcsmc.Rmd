...
* Other (unbiased) resampling schemes have been proposed in the literature which have smaller variance in the number of offspring of each particle than by multinomial resampling, in descending order of popularity/efficiency
    ...
    + Residual resampling
    + Multinomial resampling (Murray suggests to stick to this)
...
Algorithm $\texttt{dcsmc}$

>
1. For $c \in \mathcal{C}(t)$:
    (a) $(\{x_c^i, w_c^i\}_{i=1}^N, \hat{Z}_c^N) \leftarrow \texttt{dcsmc}(c)$
    (b) Resample $\{x_c^i, w_c^i\}_{i=1}^N$ to obtain the equally weighted particle system $\left\{\check{x}_c^i, 1\right\}_{i=1}^{N}$
4. For particle $i = 1, \ldots, N$:
    (a) If $\widetilde{\mathcal{X}}_t \neq \emptyset$, simulate $\widetilde{x}_t^i \sim q_t\left(\cdot | \check{x}_{c_1}^i, \ldots, \check{x}_{c_C}^i\right)$, where $(c_1, c_2, \ldots, c_C) = \mathcal{C}(t)$; else $\widetilde{x}_t^i \leftarrow \emptyset$
    (b) Set $x_t^i = (\check{x}_{c_1}^i, \ldots, \check{x}_{c_C}^i, \widetilde{x}_t^i)$
    (c) Compute $\frac{\gamma_{t}\left(x_t^i\right)}{\prod_{c \in \mathcal{C}(t)} \gamma_c\left(\check{x}_c^i\right)} \frac{1}{q_t\left(\widetilde{x}_t^i | \check{x}_{c_1}^i, \ldots, \check{x}_{c_C}^i\right)}$
5. Compute $\hat{Z}_t^N = \{ \frac{1}{N} \sum_{i=1}^N w_t^i \} \prod_{c \in \mathcal{C}} \hat{Z}_c^N$
