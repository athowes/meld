...
* Markov melding with linear opinion pooling $w_1 = w_2 = 0.5$ does not appear to have a closed form density
...
logtarget <- function(x) {
  if(0 < x & x < 1) {
    return(log(dbeta(x, shape1 = tau1 + a + n2 + c, shape2 = n1*m - tau1 + b + tau2 + d)) + 
           log(dbeta(x, a, b) + dbeta(x, c, d)) - log(dbeta(x, a, b)) - log(dbeta(x, c, d))
    )
  }
    else return(-Inf)
}
...
plot4 <- ggplot() + 
  geom_histogram(aes(x = run$chain), bins = 50, fill = "#FF7F00", alpha = 0.6) +
  geom_vline(xintercept = theta, colour="grey30", linetype = "longdash") +
  xlim(c(0,1)) +
